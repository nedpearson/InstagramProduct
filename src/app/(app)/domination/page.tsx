      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 relative z-10">
         ...
         <div className="glass-panel-ai rounded-2xl flex flex-col h-[400px] overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide p-4 space-y-3">
               {[
                 { name: 'SaaS Continuity Hook', heat: 98, color: 'emerald' },
                 { name: 'Done-For-You Ascension', heat: 87, color: 'emerald' },
                 { name: 'Low Ticket Ebook', heat: 42, color: 'amber' },
                 { name: 'Group Coaching Model', heat: 24, color: 'red' },
                 { name: 'Organic DMs Automations', heat: 91, color: 'emerald' }
               ].map((item, idx) => (
                 <div key={idx} className="p-3 bg-white/[0.02] border border-white/[0.04] rounded-xl flex items-center justify-between hover:bg-white/[0.04] transition-colors">
                    <div className="flex flex-col relative w-full pt-1">
                      <div className="flex items-center justify-between w-full z-10 relative px-2 mb-2">
                        <span className="text-[12px] font-bold text-zinc-200">{item.name}</span>
                        <span className={`text-[11px] font-black tabular-nums ${item.color === 'emerald' ? 'text-emerald-400' : item.color === 'amber' ? 'text-amber-400' : 'text-red-400'}`}>{item.heat}°</span>
                      </div>
                      <div className="w-full bg-white/[0.02] h-7 absolute top-0 left-0 rounded opacity-60 overflow-hidden">
                        <div className={`h-full rounded transition-all duration-1000 ${
                          item.color === 'emerald' ? 'bg-gradient-to-r from-transparent to-emerald-500/20 border-r border-emerald-500/50' :
                          item.color === 'amber' ? 'bg-gradient-to-r from-transparent to-amber-500/20 border-r border-amber-500/50' :
                          'bg-gradient-to-r from-transparent to-red-500/20 border-r border-red-500/50'
                        }`} style={{ width: `${item.heat}%`}}></div>
                      </div>
                    </div>
                 </div>
               ))}
